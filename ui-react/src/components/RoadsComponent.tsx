...
// Простой CSV-парсер (без поддержки кавычек и вложенных запятых)
...
const computeBounds = (nodes: Record<string, any>): LatLngBoundsExpression | null => {
    ...
    if (minLat === maxLat && minLon === maxLon) {
        const delta = 0.001; // Минимальный отступ, чтобы map.fitBounds работал
        return [
...
export const RoadsComponent: React.FC<RoadsComponentProps> = ({ graphData, onDownload, isActive }) => {
    if (import.meta.env.DEV) {
        // Логируем входящие данные для диагностики только в dev-режиме
        console.log('graphData:', graphData);
    }

    // Если данные в формате CSV, парсим их
    ...
    // Центр карты по первому узлу, если есть

    // Приведение nodes к объекту для Object.values/Object.entries
    ...
    // Если нет узлов или рёбер, показываем заглушку
