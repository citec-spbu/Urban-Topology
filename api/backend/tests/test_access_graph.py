...
from api.backend.infrastructure.osm.osm_handler import (
...
class AccessGraphAssemblerTests(unittest.TestCase):
    ...
    def test_builds_access_graph_with_building_links(self):
        nodes = self._make_nodes()
        roads = self._make_roads()
        ...
        self.assertEqual(len(building_nodes), 1)
        self.assertTrue(any(edge.get("is_building_link") for edge in edge_payloads))
        road_edges = [e for e in edge_payloads if not e.get("is_building_link")]
        self.assertGreaterEqual(len(road_edges), 2)

    def test_building_ignored_when_too_far(self):
        nodes = self._make_nodes()
        roads = self._make_roads()
        ...
        self.assertFalse(any(n["source_id"] == 6001 for n in node_payloads))
        self.assertFalse(any(e.get("is_building_link") for e in edge_payloads))

    def test_polygon_centroid_handles_square(self):
        coords = [(0.0, 0.0), (1.0, 0.0), (1.0, 1.0), (0.0, 1.0)]
        centroid = _polygon_centroid(coords)
        self.assertAlmostEqual(centroid[0], 0.5, places=3)
        self.assertAlmostEqual(centroid[1], 0.5, places=3)
