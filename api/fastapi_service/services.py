...
async def property_to_scheme(property: CityProperty) -> PropertyBase:
    ...
    return PropertyBase(
        ...
        time_created=str(property.time_created),
        c_latitude=property.c_latitude,
        c_longitude=property.c_longitude,
    )
...
def add_graph_to_db(city_id: int, file_path: str, city_name: str) -> None:
    try:
        ...
        query = (
            update(CityAsync)
            .where(CityAsync.c.id == f"{city_id}")
...
def get_admin_levels(
    city: City, regions: GeoDataFrame, cities: DataFrame
) -> List[RegionBase]:
    regions_list = []
    ...
    info = regions[regions["local_name"] == city.city_name].sort_values(
        by="admin_level"
    )
...
async def graph_from_poly(city_id, polygon):
    ...
    points, edges, ways_prop_ids, points_prop_ids = filter_by_polygon(
        polygon=polygon, edges=edges, points=points
    )
    print(
        f"{datetime.now()} filter by polygon end - Points: {len(points)}, Edges: {len(edges)}, Way IDs: {len(ways_prop_ids)}, Point IDs: {len(points_prop_ids)}"
    )

    # Early return if no data after filtering
    if len(points) == 0 or len(edges) == 0:
        print(
            f"{datetime.now()} WARNING: No data found after polygon filtering for city_id={city_id}"
        )
        return [], [], [], [], {}
    ...
    # Handle point properties query
    if len(points_prop_ids) > 0:
        ...
        temp_table = Table(
            "temp_ids_point", metadata, Column("id_point", Integer, primary_key=True)
        )

        # Создайте временную таблицу
        conn.execute(
            text("CREATE TEMPORARY TABLE temp_ids_point (id_point BIGINT PRIMARY KEY);")
        )
        ...
        print(
            f"{datetime.now()} No point property IDs found, skipping point properties query"
        )
        points_prop = []
...
async def calc_metrics(points, edges, oneway_ids):
    ...
    ]  # Обратные ребра для двусторонних дорог
    ...
    G.add_edges_from(
        reversed_edges_list
    )  # Добавляем обратные направления для двусторонних дорог
